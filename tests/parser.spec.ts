...
describe('parser statements', () => {
  ...
  it('parses fetch with where and into', () => {
    ...
    expect(fetch.kind).toBe('FetchExpression');
  ...
  it('parses send and store verbs', () => {
    const ast = parse('send body to target.\nstore body into cache.');
    expect((ast.body[0] as any).expression.kind).toBe('SendExpression');
    expect((ast.body[1] as any).expression.kind).toBe('StoreExpression');
  });
